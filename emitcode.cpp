...
{
    if (TraceCode) fprintf(code, (char*)"* %s\n", c);
}
...
{
    if (TraceCode) fprintf(code, (char*)"* %s at line %d\n", c, lineno);
}
...
{
    if (TraceCode) fprintf(code, (char*)"* %s %s\n", c, cc);
}
...
{
    if (TraceCode) fprintf(code, (char*)"* %s %s at line %d\n", c, cc, lineno);
}
...
{
    fprintf(code, (char*)"%3d:  %5s  %d,%d,%d ", emitLoc++, op, r, s, t);
    if (TraceCode) fprintf(code, (char*)"\t%s %s", c, cc);
    fprintf(code, (char*)"\n");
...
{
    fprintf(code, (char*)"%3d:  %5s  %d,%d(%d) ", emitLoc++, op, r, d, s);
    if (TraceCode) fprintf(code, (char*)"\t%s %s", c, cc);
    fprintf(code, (char*)"\n");
    if (highEmitLoc<emitLoc) highEmitLoc = emitLoc;
}
...
{
    fprintf(code, (char*)"%3d:  %5s  %d,%d(%d) ", emitLoc, op, r, a - (emitLoc + 1),
	    PC);
    emitLoc++;
    if (TraceCode) fprintf(code, (char*)"\t%s %s", c, cc);
    fprintf(code, (char*)"\n");
